...
export class FreelancerService {
  ...
  getFreelancerByEmail(email: string): Observable<BackendFreelancer | null> {
    return this.getAllFreelancersRaw().pipe(
      map(list => list.find(f => f.email?.toLowerCase() === email.toLowerCase()) || null),
      catchError(err => {
        console.error('Error fetching freelancer by email', err);
        return of(null);
      })
    );
  }
